    ...
    fn validate(field: &Field, hand: &WinningHand, _: &Status) -> Option<YakuEntry> {
        for mentsu in hand.hand {
            match mentsu {
                ...
                Mentsu::Janto(tile) => {
                    if !Self::is_valid_janto(&tile, field) {
                        return None;
                    }
                }
                _ => return None,
        ...
        match tile.tile_type {
            ...
            TileType::Wind => {
                if is_same_wind(tile.number, &field.bakaze)
                    || is_same_wind(tile.number, &field.zikaze)
                {
                    return false;
                }
                true
...
mod invalid {
    ...
    use crate::finder::ii_han::pinfu::Pinfu;
    use crate::finder::test_utils::{
        random_field, random_janto, random_shuntu, random_shuntu_number, random_status,
        random_tile, random_tile_type,
    };
    use rand::{random, Rng};
    ...
    fn head_is_yakuhai_dragon() {
        ...
        let hand = [
            ...
            Mentsu::Janto(Tile {
                ...
                number: rand::thread_rng().gen_range(1..=3),
            }),
        ];
        let winning_tile = hand[0].tile();
        let winning_hand = WinningHand {
